...
public class NightAuditDAOImp implements NightAuditDAO {
	...
	public JsonArray getArrivalList(){
		...
		String sqlArrivalList = "select rh.resv_no,rh.folio_bind_no,rd.arr_date,rh.resv_by_first_name as  firstName,rh.resv_by_last_name,rh.corporate_name,rh.resv_by_mail as email,rh.resv_by_phone as phone,rh.resv_date,rh.`status`,rd.resv_dtl_no,rd.room_type_id,rd.room_type_code,rd.rate_code,rd.occupancy,r.name as roomName,rm.resv_room_no,rm.room_number,rm.is_noshow,rm.noshow_date,rm.noshow_reason,rm.first_name,rm.last_name,rm.gender, rm.address,rm.email,rm.phone,rm.nationality from resv_hdr rh inner join resv_dtl rd on rh.resv_no=rd.resv_no inner join resv_room rm on rd.resv_dtl_no=rm.resv_dtl_no left join room r on  rm.room_number=r.number where rh.`status` in (0,1,3,4) and rm.room_status in(0,1) and rd.arr_date=(select system_setting.hotel_date from system_setting)";
		try {	
	...
	public JsonObject getRoomRateForCurrentDate(String hotelDate,int checkInNo){
		...
		System.out.println("hoteldate"+hotelDate);
		System.out.println("checkin"+checkInNo);
		try {		
