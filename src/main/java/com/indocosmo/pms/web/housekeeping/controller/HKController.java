...
public class HKController {
	...
	@RequestMapping(value = "/hkStatusCount", method = RequestMethod.POST)
	public @ResponseBody String hkStatusCount(HttpSession session, @RequestBody String data) throws Exception {
		JsonParser jsonParser = new JsonParser();
		JsonObject jsonObject = jsonParser.parse(data).getAsJsonObject();
		int roomType = jsonObject.get("roomType").getAsInt();
		int floor = jsonObject.get("floor").getAsInt();
		...
		JsonArray defaultValues  =jsonObject.get("defaultValues").getAsJsonArray();

	    List<RoomStatusCount> roomDtlList = null;
	    Gson gson = new Gson();
	    try {
	        roomDtlList = dashboardService.getRoomStatusCount(roomType,floor,defaultValues);
	    } catch (Exception e) {
	        e.printStackTrace();
	        throw new CustomException();
	    }
