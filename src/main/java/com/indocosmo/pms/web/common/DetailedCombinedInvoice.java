...
public class DetailedCombinedInvoice extends AbstractITextPdfView {
	...
	protected void buildPdfDocument(Map<String, Object> model, Document doc, PdfWriter writer,
		...
		if (txn != null) {
			...
			cellchk.setPhrase(new Phrase("GRC No       : ", fontHead));
			tablechk.addCell(cellchk);
			lineCount++;
			cellchk.setPhrase(new Phrase(listInvoice.getGrcNo(), fontHead));
			tablechk.addCell(cellchk);
			...
			for (Transaction txnInvoice : txnListInvoice) {
				...
				if (txnInvoice.getAcc_mst_id() != 4 && txnInvoice.getAcc_mst_id() != 5
					...
					//data invoice 
					if(txnInvoice.isDiscApplied()) {
						
						if(txnInvoice.getSource_folio_no() == 0 ) {
							...
							cell.setHorizontalAlignment(Element.ALIGN_LEFT);
							cell.setPhrase(new Phrase(txnInvoice.getAccMstName() +"("+txnInvoice.getSourceRoom()+")", font2));
							tableNew.addCell(cell);
						...
						cell.setPhrase(new Phrase(formatter.format(txnInvoice.getActualBaseAmount()), font2));
						tableNew.addCell(cell);
						...
						cell.setHorizontalAlignment(Element.ALIGN_LEFT);
						cell.setPhrase(new Phrase(txnInvoice.getDiscount()+"(-"+txnInvoice.getDisc_amount()+")", font2));
						tableNew.addCell(cell);
				...
				if (rowCount == txnListInvoice.size()) {
					...
					for (int i = rowCount + DisrowCount; i <= 12; i++) {
...
// rowCount changed to rowCount + DisrowCount
					for (int i = rowCount + DisrowCount; i == 15; i++) {
						PdfPCell emptyCell = new PdfPCell();
						emptyCell.setHorizontalAlignment(Element.ALIGN_RIGHT);
						emptyCell.setBorder(Rectangle.LEFT | Rectangle.RIGHT);
			...
			tableSummaryCell.setPhrase(new Phrase(String.valueOf(formatter.format(listInvoice.getTotal()+listInvoice.getFoodExpense())), font));
			table_summary.addCell(tableSummaryCell);
		...
		/*	if (listInvoice.getFoodExpense() != 0) {
			...
			if (listInvoice.getGrandTotal() == 0.0) {
				...
				/*cellSettlement.setPhrase(new Phrase("Amount in words: " + MoneyToWordsConvertor.toWords(
						...
						fonttxhdr));*/
				cellSettlement.setPhrase(new Phrase("Amount in words(Rounded): " + MoneyToWordsConvertor.toWords(
				new BigDecimal(Math.round(listInvoice.getGrandTotal())).doubleValue()),
				fonttxhdr));
			...
			if (listInvoice.getDeposit() != 0 && (listInvoice.getGrandTotal()<listInvoice.getDeposit())) {
				cellSettlement.setBorder(Rectangle.RIGHT | Rectangle.TOP | Rectangle.LEFT);
				cellSettlement.setPhrase(new Phrase("Refund Amount", font));
				tableSettlement.addCell(cellSettlement);
				 refund_amt=listInvoice.getDeposit()-listInvoice.getGrandTotal();
				cellSettlement.setPhrase(new Phrase(String.valueOf(formatter.format(refund_amt)), font));
				tableSettlement.addCell(cellSettlement);
			}
			...
			cellSettlement.setPhrase(new Phrase(
					String.valueOf(
							formatter.format(paidAmount + listInvoice.getDeposit() + listInvoice.getRoundAdjust() - listInvoice.getGrandTotal()-refund_amt)),
					fontAmt));
			...
			cellSettlement.setColspan(2);
			cellSettlement.setHorizontalAlignment(Element.ALIGN_LEFT);
			cellSettlement.setPhrase(new Phrase(" Bank Details\n"+ " Account Name      : Hikari Hotels Pvt Ltd\n" + 
					" Account No           : 13165600005538\n"  + 
					" Bank & Branch      : Federal Bank , Panampilly Nagar\n"+ 
					" IFSC                       : FDRL0001316\n"   
					,font3));			//cellSettlement.setPhrase(new Phrase(String.valueOf(formatter.format), font));
			tableSettlement.addCell(cellSettlement);
			cellSettlement.setPhrase(new Phrase("", fontAmt));
			tableSettlement.addCell(cellSettlement);
