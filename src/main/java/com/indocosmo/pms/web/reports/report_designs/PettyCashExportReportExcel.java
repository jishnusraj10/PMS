...
public class PettyCashExportReportExcel extends AbstractExcelView {
		...
		if (pettyExpenseReport.size() != 0) {
			...
			for (int i = 0; i < pettyExpenseReport.size(); i++) {
				...
				if(pettyCash.getVoucherType().equals("PAYMENT")) {
					...
					HSSFCell cellAmount = detailRow.createCell(1);
					cellAmount.setCellStyle(contentCellStyle);
					cellAmount.setCellValue("To");
				}
				...
				if((pettyCash.getVoucherType().equals("PAYMENT"))  || (pettyCash.getVoucherType().equals("CONTRAC"))){
			...
			closinBlnce = debitTotal-creditTotal;	
			...
			cellSiNo6.setCellValue(" ");
				...
				if(j == 1 ) {
					...
					cellSiNo7.setCellValue(String.valueOf(debitTotal));
				}
				else if( j == 2){
				...
				if(j == 1 ) {
					...
					cellSiNo8.setCellValue(String.valueOf(closinBlnce));
					
				}
				else {
					cellSiNo8.setCellValue(String.valueOf(debitTotal));
