...
<c:set var="moduleTitle" value="Petty Expense Head" scope="request" />
<c:set var="moduleName" value="Petty Expense Head" scope="request" />
<c:set var="backUrl" value="/pettycash/list" scope="request" />
<c:set var="backBtnStatusVal" value="1" scope="request" />
<c:set var="customEditIncludeFile"
	value="../pettycash/petty_form.jsp" scope="request" />
	
	<c:set var="masterListHeaderName" value="Petty Expense Head"
	scope="request" />
...
<link rel="shortcut icon"
	...
	src="<c:url value='/resources/pms/js/pettycash.js' />"></script>
	<script type="text/javascript"
	src="<c:url value='/resources/common/js/pms_edit_common.js'/>"></script>
	</head>
	<body ng-controller="pettyCashController" class="full-width" id="tools">
