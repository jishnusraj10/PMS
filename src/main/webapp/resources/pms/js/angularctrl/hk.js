...
pmsApp.controller('hkController',['$scope','$http','$mdDialog','$compile','$q','hkService', function($scope,$http,$mdDialog,$compile,$q,hkService){
	...
	$scope.gethkstatuscount = function(){
		...
		if ($scope.hk_filter.cleaning) {
			defaultValues.push(2);
		}
		if ($scope.hk_filter.clean) {
			defaultValues.push(3);
		}
		
		$http({
			url : '../housekeeping/hkStatusCount',
			method : "POST",
			data :{roomType:$scope.rmType,
					floor:$scope.floor,
					defaultValues: defaultValues}
			
		}).then(function(response) {
			$scope.OccupiedCount = response.data[0].Occupied;
			$scope.VacantCount = response.data[0].Vacant;
			$scope.OutInventoryCount = response.data[0].OutInventory;
			$scope.DirtyCount = response.data[0].Dirty;
			$scope.CleaningCount = response.data[0].Cleaning;
			$scope.CleanCount = response.data[0].Clean;
